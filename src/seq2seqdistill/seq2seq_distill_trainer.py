...
from trainer import DistillationTrainingArguments, DistillationTrainer
from transformers import DataCollatorForSeq2Seq
from utils import load_teacher_model, load_student_model, load_tokenizer, load_distill_dataset
from cosine_annealing_warmup import CosineAnnealingWarmupRestarts
# from custom import CosineAnnealingWarmUpRestarts
import torch
...
class Seq2SeqDistillTrainer:
    def __init__(self, args):
        ...
        self.dataset_data_type = args.get("dataset_data_type", None)
        ...
        self.warmup_steps = args.get("warmup_steps", None)
        self.learning_rate = args.get("learning_rate", None)
        # self.lr_scheduler_type = args.get("lr_scheduler_type", None)
        ...
        # load student model
        if args.get("student", None):
            self.student_model = T5ForConditionalGeneration.from_pretrained(args.get("student", None))
        else:
        ...
        if self.model_type == "bart":
            self.optimizer = torch.optim.AdamW(self.student_model.parameters(), lr=self.learning_rate)
        elif self.model_type == "t5":
            self.optimizer = torch_optimizer.Adafactor(self.student_model.parameters(), lr=self.learning_rate)
        else:
            ValueError("model_type must be bart or t5")
        self.lr_scheduler_type = CosineAnnealingWarmupRestarts(self.optimizer, 
        ...
        # embedding copy and freeze
        # self.student_model = copy_layers(self.teacher_model, self.student_model)
    ...
    def train(self):
        ...
        self.trainer = DistillationTrainer(
            ...
            optimizers=(self.optimizer, self.lr_scheduler_type)
        )
        # start training
        self.trainer.train()
        # save model
